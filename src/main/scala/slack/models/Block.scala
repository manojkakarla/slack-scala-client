...
object BlockElement {
  ...
  private implicit val optionObjFmt = Json.format[OptionObject]
  private implicit val optionGrpObjFmt = Json.format[OptionGroupObject]
  private implicit val confirmObjFmt = Json.format[ConfirmationObject]

  private implicit val eitherOptFmt = eitherObjectFormat[OptionObject, OptionGroupObject]("text", "label")
  private implicit val buttonElementFmt = Json.format[ButtonElement]
  private implicit val imageElementFmt = Json.format[ImageElement]
  private implicit val staticMenuElementFmt = Json.format[StaticSelectElement]
  private implicit val extMenuElementFmt = Json.format[ExternalSelectElement]
  private implicit val userMenuElementFmt = Json.format[UserSelectElement]
  private implicit val channelMenuElementFmt = Json.format[ChannelSelectElement]
  private implicit val conversationMenuElementFmt = Json.format[ConversationSelectElement]
  private implicit val overflowElementFmt = Json.format[OverflowElement]
  private implicit val datePickerElementFmt = Json.format[DatePickerElement]

  private val elemWrites = new Writes[BlockElement] {
    def writes(element: BlockElement) = {
  ...
  private val elemReads = new Reads[BlockElement] {
    def reads(jsValue: JsValue): JsResult[BlockElement] = {
      ...
      value match {
        ...
        case "channelss_select" => jsValue.validate[ChannelSelectElement]
...
object Block {
  ...
  private implicit val eitherContextFmt = eitherObjectFormat[ImageElement, TextObject]("image_url", "text")
  private implicit val dividerFmt = Json.format[Divider]
  private implicit val imageBlockFmt = Json.format[ImageBlock]
  private implicit val actionBlockFmt = Json.format[ActionsBlock]
  private implicit val contextBlockFmt = Json.format[ContextBlock]
  private implicit val sectionFmt = Json.format[Section]
